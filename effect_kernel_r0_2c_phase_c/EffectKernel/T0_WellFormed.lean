...
private theorem grantWF_gamma_eq {s t : State} (h : t.gamma = s.gamma)
  ...
  · intro g gr p htg hparent
    have hsg : s.gamma g = some gr := by
      rw [← h]
      exact htg
    rcases hw.parent_ok g gr p hsg hparent with ⟨pgr, hp, hgen, hroot⟩
    refine ⟨pgr, ?_, hgen, hroot⟩
    rw [h]
    exact hp
...
private theorem delegate_grantWF {s t : State} {parent child : GrantID} {rec : GrantRecord}
  ...
  · intro g gr p htg hparent
    ...
    · subst g
      ...
      subst p
      rcases hd.parentPre with ⟨pgr, hp, hgen, _hpv, hrootEq⟩
      refine ⟨pgr, ?_, hgen, hrootEq⟩
      rw [hd.preserveOther parent hpc]
      exact hp
    · have hsg : s.gamma g = some gr := by
        rw [← hd.preserveOther g hgc]
        exact htg
      rcases hw.parent_ok g gr p hsg hparent with ⟨pgr, hp, hgen, hrootEq⟩
      have hpc' : p ≠ child := by
        intro h
        subst p
        rw [hd.fresh] at hp
...
private theorem revoke_grantWF {s t : State} {g : GrantID}
  ...
  rcases hr.post with ⟨old, new, hsold, htnew, hroot, hpar, _hpv, hgen, _hver,
  ...
  · intro x gr htx hnone
    ...
    · subst x
      have heq : gr = new := option_some_inj (htx.symm.trans htnew)
      subst gr
      have holdnone : old.parent = none := by rw [← hpar]; exact hnone
      have holdroot := hw.root_ok g old hsold holdnone
      exact hroot.trans holdroot
    · have hsx : s.gamma x = some gr := by rw [← hr.preserveOther x hx]; exact htx
  ...
  · intro x gr p htx hparent
    ...
    · subst x
      have heq : gr = new := option_some_inj (htx.symm.trans htnew)
      subst gr
      have holdpar : old.parent = some p := by rw [← hpar]; exact hparent
      rcases hw.parent_ok g old p hsold holdpar with ⟨pgr, hp, hlt, hrootEq⟩
      by_cases hpg : p = g
      · subst p
        have impossible : old.generation < old.generation := by
          have heq2 : pgr = old := option_some_inj (hp.symm.trans hsold)
          subst pgr
          exact hlt
        exact False.elim (Nat.lt_irrefl _ impossible)
      · refine ⟨pgr, ?_, ?_, ?_⟩
        · rw [hr.preserveOther p hpg]; exact hp
        · rw [hgen]; exact hlt
        · rw [hroot]; exact hrootEq
    · have hsx : s.gamma x = some gr := by rw [← hr.preserveOther x hx]; exact htx
      rcases hw.parent_ok x gr p hsx hparent with ⟨pgr, hp, hlt, hrootEq⟩
      by_cases hpg : p = g
      · subst p
        refine ⟨new, htnew, ?_, ?_⟩
        · have heq : pgr = old := option_some_inj (hp.symm.trans hsold)
          subst pgr
          rw [hgen]
          exact hlt
        · have heq : pgr = old := option_some_inj (hp.symm.trans hsold)
...
private theorem attenuate_grantWF {s t : State} {g : GrantID}
  ...
  rcases ha.post with ⟨old, new, hsold, htnew, hroot, hpar, _hpv, hgen, _hsub,
  ...
  · intro x gr htx
    ...
    · subst x
      ...
      subst gr
      exact hvalidwf
    · have hsx : s.gamma x = some gr := by rw [← ha.preserveOther x hx]; exact htx
      exact hw.interval_ok x gr hsx
  · intro x gr htx hnone
    by_cases hx : x = g
    · subst x
  ...
  · intro x gr p htx hparent
    ...
    · subst x
      ...
      · subst p
        ...
        exact False.elim (Nat.lt_irrefl _ impossible)
      · refine ⟨pgr, ?_, ?_, ?_⟩
        · rw [ha.preserveOther p hpg]; exact hp
        · rw [hgen]; exact hlt
        · rw [hroot]; exact hrootEq
    · have hsx : s.gamma x = some gr := by rw [← ha.preserveOther x hx]; exact htx
      rcases hw.parent_ok x gr p hsx hparent with ⟨pgr, hp, hlt, hrootEq⟩
...
private theorem updatePolicy_budgetOwners {m : PolicyMutation} {s t : State}
    {newPolicy : GovernancePolicy} {mid : MetaAuthID}
    (h : UpdatePolicy m s t newPolicy mid)
    (hw : BudgetOwnersWellFormed s) : BudgetOwnersWellFormed t := by
  cases m with
  | nonExpanding =>
      have hb : t.budget = s.budget := by
        simpa using h.budgetRule
      exact budgetOwners_same h.gammaSame hb hw
  | positiveProvision r delta =>
      have hb : ProvisionBudget s t r delta := by
        simpa using h.budgetRule
      exact budgetOwners_one_changed h.gammaSame hb.rootExists hb.other hw
...
theorem trustedStep_preserves_wf {s t : State} {kind : TransitionKind}
  ...
  | updatePolicy h =>
      exact ⟨grantWF_gamma_eq h.gammaSame hw.grants,
        updatePolicy_budgetOwners h hw.budgetOwners⟩
  | expireReservation h =>
      exact ⟨grantWF_gamma_eq h.same.gamma hw.grants,
        budgetOwners_one_changed h.same.gamma h.grantExists h.budgetUpdate.other hw.budgetOwners⟩
  | expireGrant h =>
...
theorem ancestor_generation_decreases {s : State} (hw : WellFormed s)
  ...
  | direct hchild hparent =>
      rcases hw.grants.parent_ok _ _ _ hchild hparent with ⟨pgr, hp, hlt, _hroot⟩
      exact ⟨pgr, _, hp, hchild, hlt⟩
  | extend hprefix hchild hparent ih =>
      rcases ih with ⟨agr, pgr, ha, hp, hap⟩
      rcases hw.grants.parent_ok _ _ _ hchild hparent with ⟨pgr2, hp2, hpc, _hroot⟩
